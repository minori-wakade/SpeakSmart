    ...
    # -----------------------------------------
        ...
        with st.spinner("Analyzing your video..."):
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                futures = {
                    # Thread 1 → Audio
                    executor.submit(run_audio_pipeline, audio_path): "audio",

                    # Thread 2 → Eye Contact
                    executor.submit(
                        process_eye_contact,
                        file_path,
                    ...
                    # Thread 3 → Posture + Face (Sequential but in one worker)
                    executor.submit(
                        lambda: {
                            "posture": run_posture_analysis(file_path),
                            "face": process_video(file_path)
                        }
                    ): "video_misc",
                }
        ...
        audio_result = results["audio"]
        eye_result = results["eye"]

        video_misc = results["video_misc"]
        posture_result = video_misc["posture"]
        deepface_result = video_misc["face"]
        ...
        # Video
        eye_contact_percentage, eye_status, eye_feedback, _ = eye_result
